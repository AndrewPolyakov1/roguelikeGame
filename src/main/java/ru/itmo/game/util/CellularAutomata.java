...
public class CellularAutomata {
     ...
     */
    public CellularAutomata(int height, int width) {
        this.width = width;
        this.height = height;
    ...
    public int numWallNeighbors(Point coordOfCell) {
        ...
        // neighbor 4 @ (x - 1, y)
        if (x - 1 < 0 || cellGrid[x - 1][y] == WALL) {
            numWallNeighbors += 1;
        }
        // neighbor 5 @ (x + 1, y)
        if (x + 1 >= width || cellGrid[x + 1][y] == WALL) {
            numWallNeighbors += 1;
        }
        // neighbor 6 @ (x - 1, y + 1)
        ...
        // neighbor 7 @ (y + 1)
        if (y + 1 >= height || cellGrid[x][y + 1] == WALL) {
            numWallNeighbors += 1;
        }
        // neighbor 8 @ (x + 1, y + 1)
